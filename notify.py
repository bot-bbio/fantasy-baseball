...
def build_message(subject: str, body: str, settings: config.Settings) -> EmailMessage:
...
def send_email(subject: str, body: str, settings: config.Settings) -> bool:
    """Send one email. Returns False (no-op) if email isn't configured."""
    if not settings.email_enabled:
        return False
    message = build_message(subject, body, settings)
